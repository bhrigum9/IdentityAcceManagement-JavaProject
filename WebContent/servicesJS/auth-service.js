(function() {
		...
		function Login(username, password, callback) {
			...
			$timeout(
					function() {
						var response;
						$http
								.post('/iamProject/rest/UserService/auth', {
									username : username,
									password : password
								})
								.then(
										function(user) {
											...
											if (user.config.data.username === username
													&& user.config.data.password === password) {
												response = {
													success : true,
													userid : userid
												};
	...
	var Base64 = {
		...
		decode : function(input) {
			...
			var enc1, enc2, enc3, enc4 = "";
			var i = 0;
			// remove all characters that are not A-Z, a-z, 0-9, +, /, or = var
